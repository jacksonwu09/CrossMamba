...
def loadweight2changedyolo(model_dict, pt_dict, source_orders, target_orders, exclude=()):
    # 将官方权重按模块序号转移到修改后的模型结构中
    """
    :param model_dict: 模型的参数名字典
    :param pt_dict:  要导入的权重字典
    :param source_orders: 权重字典中要导入部分的序号集，可指定多段，例如[(0，3)， (5, 10)]代表将0-3序号的模块和5-10序号的模块提取导入
    :param target_orders: 模型中要加载参数部分的序号集，必须与权重提取的段数相同，例如[(1,4), (12,17)]，
    :param exclude: 过滤项，将包含exclude中字符串的权重项忽略
    :return: 返回模型中所有要加载的参数以及其对应值的字典
    """
    assert len(source_orders) == len(target_orders), "source_order length is not match with target_order length"
    model_dict_name = model_dict.keys()
    pt_dict_name = pt_dict.keys()
    csd = {}
    for source_order, target_order in zip(source_orders, target_orders):
        source_order_start, source_order_end = source_order
        target_order_start, tartget_order_end = target_order

        source_name = [f"model.{i}." for i in range(source_order_start, source_order_end)]
        source_names = [weights_name for name in source_name for weights_name in pt_dict_name if name in weights_name]

        target_name = [f"model.{i}." for i in range(target_order_start, tartget_order_end)]
        target_names = [weights_name for name in target_name for weights_name in model_dict_name if name in weights_name]

        # assert len(source_names) == len(target_names), "source_name length is not match with target_name length"

        for i in range(len(source_names)):
            if pt_dict[source_names[i]].shape == model_dict[target_names[i]].shape and all(x not in source_names[i] for x in exclude):
                csd[target_names[i]] = pt_dict[source_names[i]]
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    # Model
    Model = Model_mono if opt.mono else Model_multi
    ...
    if pretrained:
        ...
        csd = loadweight2changedyolo(model.state_dict(), csd, exclude=exclude,
                                     source_orders=[(0, 5), (0, 5), (5, 7), (5, 7), (7, 10), (7, 10), (10, 25)],
                                     target_orders=[(0, 5), (10, 15), (5, 7), (15, 17), (7, 10), (17, 20), (23, 38)])
            ...
            if opt.mono:
                ...
                if flage_visual:
                    ...
                    for num in range(batch_size):
                        image = imgs[num, :, :, :].cpu().clone()  # clone the tensor
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='0525', help='save to project/name')
