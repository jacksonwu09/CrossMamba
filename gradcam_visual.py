...
parser.add_argument('--model-path', type=str, default="/root/autodl-tmp/CrossMamba/saves/FLIR_CrossMamba/CrossMamba3/weights/best.pt", help='Path to the model')
parser.add_argument('--source1', type=str, default='/root/autodl-tmp/CrossMamba/datasets/FLIR-align-3class/visible/test', help='source')  # file/folder, 0 for webcam
parser.add_argument('--source2', type=str, default='/root/autodl-tmp/CrossMamba/datasets/FLIR-align-3class/infrared/test', help='source')  # file/folder, 0 for webcam
parser.add_argument('--output-dir', type=str, default='/root/autodl-tmp/CrossMamba/runs/Grad_CAM_visual', help='output dir')
...
def main(img_vis_path, img_ir_path):
    ...
    if args.method == 'gradcam':
        for layer in args.target_layer:
            ...
            # ----------- 显式释放内存，每个layer结束后 ---------------
            saliency_method.remove_hooks()   # <--- 必须有！
            del saliency_method, masks, logits, boxes, class_names, confs, heat_map, final_image, heat, res_img
...
if __name__ == '__main__':
    if os.path.isdir(args.source1):
        ...
        for item in img_vis_list:
            ...
            if args.source1 == '/root/autodl-tmp/CrossMamba/datasets/FLIR-align-3class/visible/test':
